...
export default function Payment({ paymentId, paymentReceiver, price }) {
  ...
  const payNow = () => {
    ...
    axios
      .post(
        `${process.env.REACT_APP_BACKEND_SERVER}/mypatient`,
        { id: paymentId ,
          date},
        {
          headers: {
    ...
    axios
      .post(
        `${process.env.REACT_APP_BACKEND_SERVER}/conversation`,
  ...
  const [success, setSuccess ] = useState(false)
    ...
    if(!error) {
        try {
            ...
            const response = await axios.post(`${process.env.REACT_APP_BACKEND_SERVER}/payment`, {
                amount: price*100,
                id
            })
